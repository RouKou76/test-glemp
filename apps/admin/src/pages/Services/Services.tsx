  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="space-y-3">
        {services.length === 0 ? (<div className="flex flex-col items-center justify-center py-16 text-gray-300 dark:text-white/20"><p className="text-4xl mb-3">⚡</p><p className="text-sm">Нет услуг</p></div>) : services.map(service => (
          <div key={service.id} className={`bg-white dark:bg-[#1a1d27] border rounded-2xl p-4 space-y-3 shadow-sm transition-opacity ${service.active ? 'border-gray-100 dark:border-white/10' : 'border-gray-100 dark:border-white/5 opacity-50'}`}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-sm font-bold text-gray-800 dark:text-white">{service.name}</p><p className="text-xs text-gray-400 dark:text-white/40 mt-0.5">{service.price ?? 'Цена не указана'} · {ROLE_LABELS[service.assignedTo]}</p></div>
              </div>
            ...
            {Object.entries(service.fields).filter(([, f]) => f?.enabled).map(([key, f]) => (<div key={key} className="text-xs text-gray-400 dark:text-white/40 flex items-center gap-2"><span>{FIELD_LABELS[key as keyof ServiceFieldConfig]}</span>{f?.label && <span className="text-gray-300 dark:text-white/20">→ «{f.label}»</span>}</div>))}
            <div className="flex gap-2 pt-1">
              <button onClick={() => openEdit(service)} className="flex-1 py-2 rounded-xl border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/50 text-xs font-medium hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">✏️ Редактировать</button>
        ...
        <div className="fixed inset-0 z-40 bg-black/60 flex items-end" onClick={() => setShowForm(false)}>
          <div className="w-full bg-gray-50 dark:bg-[#1a1d27] rounded-t-3xl p-6 space-y-4 max-h-[90vh] overflow-y-auto animate-slide-up" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button onClick={() => setShowForm(false)} className="py-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/50 text-sm font-medium hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">Отмена</button>

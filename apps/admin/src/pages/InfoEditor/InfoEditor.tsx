  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-4 pt-4 pb-3 shrink-0">
        ...
        <p className="text-xs text-gray-400 dark:text-white/30 mt-1">Отображается в разделе «Инфо» на планшете гостя</p>
      </div>
      <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-5">
        <div><label className="text-xs font-bold text-gray-500 dark:text-white/40 uppercase tracking-wider mb-2 block">Пароль Wi-Fi</label><input type="text" value={draft.wifi} onChange={e => setDraft(p => ({ ...p, wifi: e.target.value }))} className="w-full bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-glamp-500" /></div>
        <div><label className="text-xs font-bold text-gray-500 dark:text-white/40 uppercase tracking-wider mb-2 block">Правила проживания</label><textarea value={draft.rules} onChange={e => setDraft(p => ({ ...p, rules: e.target.value }))} rows={5} className="w-full bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-glamp-500 resize-none" /></div>
        <div><label className="text-xs font-bold text-gray-500 dark:text-white/40 uppercase tracking-wider mb-2 block">Описание территории</label><textarea value={draft.description} onChange={e => setDraft(p => ({ ...p, description: e.target.value }))} rows={4} className="w-full bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-glamp-500 resize-none" /></div>
      </div>
      <div className="px-4 py-4 border-t border-gray-200 dark:border-white/10 bg-white dark:bg-[#1a1d27] shrink-0 space-y-2 transition-colors">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={handleReset} disabled={!isDirty} className="py-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/50 text-sm font-medium disabled:opacity-30 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">Сбросить</button>

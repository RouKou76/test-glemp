  ...
  return (
    <div className="h-full flex flex-col">
      <div className="bg-white dark:bg-[#1a1d27] p-4 border-b border-gray-100 dark:border-white/10 shadow-sm flex items-center gap-3 transition-colors">
        ...
        <div>
          <h2 className="font-bold text-base text-gray-800 dark:text-gray-200">Администратор</h2>
          <p className="text-xs text-green-600 dark:text-green-400 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-500 inline-block"></span> В сети</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-glamp-50 dark:bg-[#0f1117] transition-colors">
        {messages.length === 0 && <p className="text-center text-gray-500 dark:text-gray-400 text-sm mt-10">Начните диалог с администратором</p>}
        {messages.map(m => (
      ...
      <div className="p-3 bg-white dark:bg-[#1a1d27] border-t border-gray-200 dark:border-white/10 transition-colors">
        <div className="flex gap-2 bg-gray-50 dark:bg-white/5 p-1.5 rounded-full border border-gray-200 dark:border-white/10">
          <input type="text" value={msg} onChange={e => setMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()}
            placeholder="Написать сообщение..." className="flex-1 bg-transparent px-3 outline-none text-base text-gray-800 dark:text-gray-200 placeholder:text-gray-500 dark:placeholder:text-gray-500" />
          <button onClick={send} className="w-10 h-10 bg-glamp-600 text-white rounded-full flex items-center justify-center hover:bg-glamp-700 shadow-md active:scale-95 transition-all">

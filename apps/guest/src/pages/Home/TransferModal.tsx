    ...
    <Modal open={open} onClose={handleClose} title="Трансфер">
      {step === 'edit' && (
        <div className="p-6 space-y-5">
          <div>
            <p className="text-xs font-bold text-gray-500 dark:text-white/30 uppercase tracking-wider mb-3">Направление</p>
            <div className="space-y-2">
              {mockTransferDestinations.map(dest => (
                <button key={dest.id} onClick={() => setSelectedId(dest.id)}
                  ...
                  <span className="text-sm text-gray-400 dark:text-white/40">{dest.price} ₽ · {dest.km} км</span>
                </button>
              ))}
            </div>
            <p className="text-xs text-gray-400 dark:text-white/30 mt-2">Нет нужного направления? Напишите нам в чат.</p>
          </div>
          <div>
            <p className="text-xs font-bold text-gray-500 dark:text-white/30 uppercase tracking-wider mb-2">Желаемое время подачи</p>
      ...
      {step === 'review' && selected && (
        <div className="p-6 space-y-4">
          ...
          <p className="text-xs text-gray-400 dark:text-white/30">Итоговая стоимость может отличаться. Точная цена — при выезде.</p>
      ...
      {step === 'success' && (
        <div className="p-6 text-center space-y-4">
          ...
          <div><p className="text-gray-800 dark:text-white font-bold text-lg">Заявка принята!</p><p className="text-gray-500 dark:text-white/40 text-sm mt-1">Водитель будет подан к указанному времени.</p></div>

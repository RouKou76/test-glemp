...
export function FoodModal({ open, onClose, onSubmit }: FoodModalProps) {
  ...
  return (
    <Modal open={open} onClose={handleClose} title={PERIOD_LABELS[currentPeriod]}>
      ...
      {currentPeriod === 'none' && (
        <div className="text-center py-10 text-gray-400 dark:text-white/40 px-6">
      ...
      {step === 'edit' && currentPeriod !== 'none' && (
        <div className="p-6 space-y-4">
          ...
          <div>
            <p className="text-xs font-bold text-gray-500 dark:text-white/30 uppercase tracking-wider mb-2">Место подачи</p>
      ...
      {step === 'review' && (
        <div className="p-6 space-y-4">
          <div className="space-y-2">{cartItems.map(item => (<div key={item.menuItemId} className="flex justify-between text-sm py-2 border-b border-gray-100 dark:border-white/10"><span className="text-gray-800 dark:text-white">{item.name} × {item.quantity}</span><span className="text-gray-500 dark:text-white/50">{item.price * item.quantity} ₽</span></div>))}</div>
          ...
          <p className="text-sm text-gray-400 dark:text-white/40">Место: {LOCATION_OPTIONS.find(o => o.value === location)?.label}</p>
      ...
      {step === 'success' && (
        <div className="p-6 text-center space-y-4">
          ...
          <div><p className="text-gray-800 dark:text-white font-bold text-lg">Заказ принят!</p><p className="text-gray-500 dark:text-white/40 text-sm mt-1">Мы приготовим и доставим ваш заказ.</p></div>
